...
const smsRowner = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO YO COMO OWNER DE BOT LO PUEDE USAR!!\`\`\`` }//NUMERO DE BOT
const smsOwner = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO MI OWNER LO PUEDE USAR!!\`\`\`` }//OWNER
const smsMods = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO MODERADORES Y MI OWNER LO PUEDEN USAR!!\`\`\`\`` }//MODERADORES
const smsPremium = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO ESTA DISPONIBLE PARA USUARIOS PREMIUM Y MI OWNER MAX!! PARA SER PREMIUM COMPRE UN PASE USANDO #pass premium\`\`\`` }//USUARIOS PREMIUM
...
const smsLista1 = () => { return `🌟 INFORMACIÓN DE MAXERBOT 🌟`}
...
const smsLista7 = () => { return `🐈 INSTALAR MAXERBOT 🐈`}
...
//_allantilink.js
const smsTextoYT = () => { return '🤓👍 𝗦𝘂𝗽𝗲𝗿 𝗠𝗮𝘅𝗲𝗿𝗕𝗼𝘁-𝗠𝗗 - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 '} 
...
const smsAntiSp2 = () => { return `Banear el Uso de Comandos cuando alguien realice algún tipo de Spam`}
const smsModP1 = () => { return `🌐 MODO PÚBLICO`}
const smsModP2 = () => { return `Habilitar función para que todos puedan usar MaxerBot`}
const smsModAd1 = () => { return `🛂 MODO ADMIN`}
const smsModAd2 = () => { return `Solo los Admins podrán usar MaxerBot en Grupos`}
...
//_antiviewonce.js
const smsAntiView = () => { return `*𝙅𝘼𝙅𝘼𝙅𝘼 𝙈𝙊𝙍𝙏𝘼𝙇 𝙔𝙊 𝙇𝙊 𝙑𝙀𝙊 𝙏𝙊𝘿𝙊 𝘼𝙌𝙐𝙄 𝙎𝙊𝙔 𝙐𝙉 𝘿𝙄𝙊𝙎* 😎`} 
...
const smsMalused2 = () => { return `${lenguajeGB['smsAvisoMG']()}🤓☝️ *DEBE DE USAR EL COMANDO COMO EN ESTE EJEMPLO:*\n`}
const smsMalused3 = () => { return `${lenguajeGB['smsAvisoMG']()}🤓☝️ *DEBE DE USAR EL COMANDO O RESPONDER AL MENSAJE DE ALGUIEN COMO EN ESTE EJEMPLO:*\n`}
...
//propietario(a).js
const smsJoin = (user) => { return `${packname}\n𝙀𝙎 𝙐𝙉 𝘽𝙊𝙏 𝘿𝙀 𝙒𝙃𝘼𝙏𝙎𝘼𝙋𝙋 𝙌𝙐𝙀 𝙏𝙀 𝘼𝙔𝙐𝘿𝘼𝙍𝘼 𝙍𝙀𝘼𝙇𝙄𝙕𝘼𝙍 𝘿𝙄𝙁𝙀𝙍𝙀𝙉𝙏𝙀𝙎 𝘼𝘾𝙏𝙄𝙑𝙄𝘿𝘼𝘿𝙀𝙎 🪄 𝘼𝙇 𝙋𝙍𝙄𝙑𝘼𝘿𝙊 𝙊 𝙂𝙍𝙐𝙋𝙊 𝙔 𝙏𝘼𝙈𝘽𝙄𝙀𝙉 𝙏𝙀 𝙑𝘼𝙎 𝘼 𝘿𝙄𝙑𝙀𝙍𝙏𝙄𝙍 🎈 𝘾𝙊𝙉 𝙎𝙐𝙎 𝙈𝙐𝙇𝙏𝙄𝙋𝙇𝙀𝙎 𝙁𝙐𝙉𝘾𝙄𝙊𝙉𝙀𝙎, 𝘿𝙄𝙎𝙁𝙍𝙐𝙏𝘼 𝘿𝙀 𝙈𝘼𝙓𝙀𝙍𝘽𝙊𝙏!!! 😎\n\n👍 𝙈𝘼𝙓𝙀𝙍𝘽𝙊𝙏 𝙁𝙐𝙀 𝙄𝙉𝙑𝙄𝙏𝘼𝘿𝘼 𝙋𝙊𝙍:\n@${user}`}
...
const smsJBInfo2 = () => { return `💖 *La función es estable, sí presenta algún inconveniente Comuníquese al correo: maxviaje47@gmail.com*\n💝 *Puede hacer una Donación voluntaria por PayPal: ${global.paypal}*\n\n*Muchas Gracias por el apoyo a ${global.packname}*`}
...
const smsContacto4 = () => { return 'Contacto Oficial de MaxerBot 🐈'}
