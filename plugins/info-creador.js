...
*CENTER MAXERBOT*
*maxviaje47@gmail.com*

𝙈𝙖𝙭𝙚𝙧𝘽𝙤𝙩-𝙈𝘿 - 𝘼𝙎𝙄𝙎𝙏𝙀𝙉𝘾𝙄𝘼
*${asistencia}*`
  ...
  try {
    if (/(contacto|owner|creator|propietario|dueño|dueña|propietaria|dueño|creadora|creador)/i.test(command)) {
      const count = args[1] && args[1].length > 0 ? Math.min(99999999, Math.max(parseInt(args[1]), 1)) : !args[1] || args.length < 3 ? 1 : Math.min(1, count)
        switch (type) {
          case 'nombre':
          await conn.reply(m.chat, `𝙈𝙖𝙭𝙚𝙧𝘽𝙤𝙩-𝙈𝘿 : ${gt} 𝙈𝙖𝙭𝙚𝙧𝘽𝙤𝙩-𝙈𝘿`, fkontak, m)
         // conn.reply(m.chat, "𝙈𝙖𝙭𝙚𝙧𝘽𝙤𝙩-𝙈𝘿 : 𝙈𝙖𝙭𝙚𝙧𝘽𝙤𝙩-𝙈𝘿", m, { contextInfo: { mentionedJid: [nowner] }})
            break
            case 'bio':
